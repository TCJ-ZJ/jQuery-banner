; (function($) {
	...
	$.extend({
		...
		addBannerEffect:function(name,fn){
			...
			bannerEvent.prototype[name] = function() {
				...
				fn.call({'img': this.img,'param': this.param,'status': this.status});
	...
	tool.prototype.initIndex = function(num){
		...
		$.each(me,function(index,val){
			...
			pointBox = $(val).find('.' + param.btnCls[0]).find('div');
			...
			$(val).data('lzbanner-img').hide().eq(status.sPoint).css({
				top:0,
				left:0,
				zIndex:35,
				display:'block'
	...
	tool.prototype.refresh = function(){
		...
		$.each(me,function(index,val){
			...
			status.imgW = $(val).width();

			status.imgH = $(val).height();
