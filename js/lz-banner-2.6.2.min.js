...
j.bannerExcute();}});};a.prototype.touch=function(){var k={},h={},j=this.$ele,m=this.param,g=this.status,l=g.imgLen;j.on("touchstart",function(o){o.preventDefault();
o.stopPropagation();var n=o.originalEvent.touches[0];k.x=Math.floor(n.clientX);k.y=Math.floor(n.clientY);timer=setTimeout(function(){k={};h={};},1000);
});j.on("touchmove",function(o){o.preventDefault();o.stopPropagation();var n=o.originalEvent.touches[0];h.x=Math.floor(n.clientX);h.y=Math.floor(n.clientY);
});j.on("touchend",function(o){o.preventDefault();o.stopPropagation();var n=m.dir;if((n==="left"||n==="right")&&g.flag&&k.x){(k.x-h.x<-30)&&(g.cPoint-=1);
(k.x-h.x>30)&&(g.cPoint+=1);j.bannerExcute();}else{if((n==="up"||n==="down")&&g.flag&&k.y){k.y-h.y<-30&&(g.cPoint-=1);k.y-h.y>30&&(g.cPoint+=1);j.bannerExcute();
}}k={};h={};});};var e=function(g){this.$ele=f(g);this.param=this.$ele.data("lzbanner-param");this.status=this.$ele.data("lzbanner-status");this.img=this.$ele.data("lzbanner-img");
...
if(g.flag){(j.dir=="left"||j.dir=="up")?g.cPoint++:g.cPoint--;h.bannerExcute();}};d.prototype.indexChange=function(){var g=this.status,h=g.imgLen,j=this.param;
if(!j.cycle&&g.cPoint>h-1){g.cPoint=h-1;return false;}else{if(!j.cycle&&g.cPoint<0){g.cPoint=0;return false;}else{if(j.cycle&&g.cPoint>h-1){g.cPoint=0;
return true;}else{if(j.cycle&&g.cPoint<0){g.cPoint=h-1;return true;}else{return true;}}}}};d.prototype.excuteEffect=function(){var j=this,g=this.status,l=this.param,h=this.img,k=this.$ele.get(0).bannereffect;
...
g.imgH=k.height();});k.off(".lzbanner").on("mouseenter.lzbanner",function(m){clearInterval(g.timer);g.timer=null;g.mouseH=true;}).on("mouseleave.lzbanner",function(m){l.auto&&(g.timer=setInterval(h.bannerexcute.autoPlay.bind(h.bannerexcute),l.interval));
...
}if(arrow=j.find(l.arrowCls[0])){arrow.remove();}j.removeData("lzbanner-status").removeData("lzbanner-param");});return false;};b.prototype.init=function(h,g,l,k){var j=this.$ele;
this.destory();j.lzbanner(h,g,l,k);};b.prototype.initIndex=function(g){var h=this.$ele;f.each(h,function(k,m){var j=f(m).data("lzbanner-status"),l=f(m).data("lzbanner-param"),n=f(m).find("."+l.btnCls[0]).find("div");
j.sPoint=g-1;j.cPoint=g-1;if(n){n.removeClass(l.btnCls[3]).eq(j.cPoint).addClass(l.btnCls[3]);}f(m).data("lzbanner-img").hide().eq(j.sPoint).css({top:0,left:0,zIndex:35,display:"block"});
});};b.prototype.index=function(h){var j=this.$ele,g=this;if(h===undefined){return j.eq(0).data("lzbanner-status").sPoint+1;}else{f.each(j,function(l,n){g.stop();
var k=f(n).data("lzbanner-status"),m=f(n).data("lzbanner-param");if(k.flag){k.cPoint=h-1;f(n).bannerExcute();}m.auto&&g.start();});return this;}};b.prototype.next=function(){var g=this,h=this.$ele;
f.each(h,function(k,m){g.stop();var j=f(m).data("lzbanner-status"),l=f(m).data("lzbanner-param");if(j.flag){j.cPoint+=1;f(m).bannerExcute();}l.auto&&g.start();
});return this;};b.prototype.prev=function(){var g=this,h=this.$ele;f.each(h,function(k,m){g.stop();var j=f(m).data("lzbanner-status"),l=f(m).data("lzbanner-param");
if(j.flag){j.cPoint-=1;f(m).bannerExcute();}l.auto&&g.start();});return this;};b.prototype.setInterval=function(h){var j=this.$ele,g=this;f.each(j,function(k,m){g.stop();
var l=f(m).data("lzbanner-param");h>l.speed&&(l.interval=~~h);l.auto&&g.start();});return this;};b.prototype.setSpeed=function(g){var h=this.$ele;f.each(h,function(j,l){var k=f(l).data("lzbanner-param");
...
};b.prototype.offCycle=function(){var g=this.$ele;f.each(g,function(h,j){f(j).data("lzbanner-param").cycle=false;});return this;};b.prototype.refresh=function(){var g=this.$ele;
f.each(g,function(j,k){var h=f(k).data("lzbanner-status");h.imgW=f(k).width();h.imgH=f(k).height();});};b.prototype.on=function(j,g){var h=this.$ele;if(typeof g==="function"){f.each(h,function(k,l){j==="switch"&&(f(l).data("lzbanner-param")._onSwitch=g);
j==="last"&&(f(l).data("lzbanner-param")._onLast=g);});}};f.fn.bannerTool=function(){var g=f(this);return new b(g);};f.fn.bannerExcute=function(){var g=f(this).get(0);
g.bannerexcute.onProgress();};f.fn.lzbanner=function(h,g,l,k){var j=f(this);f.each(j,function(n,o){var m=new c(o,h,g,l,k);o.bannerexcute=new d(o);o.bannereffect=new e(o);
m.initEvent();m.layout();});return j.bannerTool();};})(jQuery);